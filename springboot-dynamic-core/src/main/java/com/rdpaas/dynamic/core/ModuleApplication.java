...
public class ModuleApplication {
    ...
    public void classLoader(URL url, ApplicationContext applicationContext,SqlSessionFactory sqlSessionFactory) throws Exception {
        this.applicationContext = applicationContext;
        URL[] urls = new URL[]{url};
        /**
         * 这里实际上是将spring的ApplicationContext的类加载器当成parent传给了自定义类加载器，很明自定义的子类加载器自己加载
         * 的类，parent类加载器直接是获取不到的，所以在自定义类加载器做了特殊的骚操作
         */
        TargetClassLoader targetClassLoader = new TargetClassLoader(applicationContext.getClassLoader(), urls);
        /**
         * 使用模块类加载器加载url资源的jar包，直接返回类的全限定名和Class对象的映射，这些Class对象是
         * jar包里所有.class结尾的文件加载后的结果,同时mybatis的xml加载后，无奈的放入了
         * moduleClassLoader.getXmlBytesMap()，不是很优雅
         */
        Map<String, Class> classMap = targetClassLoader.load();
         ...
         * 刷新mybatis的xml和Mapper接口资源，Mapper接口其实就是xml的namespace
         */
        Map<String, Object> extObjMap = mapperLoader.refresh(sqlSessionFactory, targetClassLoader.getXmlBytesMap());
        /**
         * 将各种资源放入spring容器
         */
        registerBeans(applicationContext, classMap, extObjMap);
